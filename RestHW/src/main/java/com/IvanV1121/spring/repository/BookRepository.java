...
public interface BookRepository extends JpaRepository<Book, Integer> {
    ...
    @Query(value = "select book_title, book_price from book where (book_title like %'Windows'%) " +
                   "or (book_price >= 20000) order by book_title asc, book_price desc", nativeQuery = true)
    ...
    @Query(value = "select book_title, book_storage, book_quantity, book_price from book b join purchase p on " +
                   "(b.book_id = p.purchase_book and b.book_quantity >= 10) join shop s on (s.shop_id = p.purchase_shop " +
